...
const controller = (function () {
  ...
  const editTodoDialogDescription = document.getElementById(
  ...
  const newProjectConfirmButton = document.getElementById(
    ...
    for (let i = 0; i < elem.length; i++) {
      elem[i].addEventListener("click", (event) => {
        if (typeof editTodoDialog.showModal === "function") {
          editTodoDialogTitle.value = currentTodoList
            .getTodoList()
            [i].getTitle();
          editTodoDialogDescription.value = currentTodoList
            .getTodoList()
            [i].getDescription();
          editTodoDateDialog.value = currentTodoList
            .getTodoList()
            [i].getDueDate();
  ...
  editConfirmDialog.addEventListener("click", () => {
    currentTodoList
      .getTodoList()
      [editIndexDialog.value].setTitle(editTodoDialogTitle.value); //using index of current todo in list edited, and sets the value entered in dialog (title)
    currentTodoList
      .getTodoList()
      [editIndexDialog.value].setDescription(editTodoDialogDescription.value);
    currentTodoList
      .getTodoList()
      [editIndexDialog.value].setDueDate(editTodoDateDialog.value);
    renderTodoList();
  });
